...
public class UsersService {
	...
	@Autowired
	public UsersService(UsersRepository usersRepository,JobSeekerProfileRepository jobSeekerProfileRepository, RecruiterProfileRepository recruiterProfileRepositry, PasswordEncoder passwordEncoder) {
		
		this.usersRepository = usersRepository;
		this.jobSeekerProfileRepository=jobSeekerProfileRepository;
		this.recruiterProfileRepositry=recruiterProfileRepositry;
	...
	public Users addNew(Users users) {
		...
		if(userTypeId==1) {
			recruiterProfileRepositry.save(new RecruiterProfile(savedUser));	
		}
		else {
	...
	public Object getCurrentUserProfile() {
		...
		if(!(authentication instanceof AnonymousAuthenticationToken)) {
			...
			if(authentication.getAuthorities().contains(new SimpleGrantedAuthority("Recruiter"))) {
				RecruiterProfile recruiterProfile=recruiterProfileRepositry.findById(userId).orElse(new RecruiterProfile());
